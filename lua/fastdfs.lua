...
if index then 
    ...
    ngx.log(ngx.STDERR, originalFile)
    index = string.find(originalFile, "([0-9]+)x([0-9]+)");
    -- local index = string.find(originalFile, "([0-9]+)x([0-9]+)");  
    ngx.log(ngx.STDERR, index)
...
if not file_exists(originalFile) then
    ...
    ngx.log(ngx.STDERR, fileid)
    -- main
    local fastdfs = require('restyfastdfs')
    local fdfs = fastdfs:new()
    fdfs:set_tracker("192.168.1.196", 22122)
...
if table.contains(image_sizes, area) then 
    local command = "gm convert -size " .. area .. " " .. originalFile  .. " -resize ".. area .."^ -background lightGray -compose Copy -gravity center -extent " .. area .. " " .. ngx.var.file;
    ngx.log(ngx.STDERR, command) 
    os.execute(command);  
end;
